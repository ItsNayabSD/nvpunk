...
M.open_win = function(bname)
    ...
    local win_w = math.ceil(vim_w * .8)
    local win_h = math.ceil(vim_h * .8 - 4)
...
M.draw = function(bname, lines)
    ...
    for i, v in ipairs(lines) do
        vim.api.nvim_buf_set_lines(M.bufs[bname], i-1, i, false, {v.message})
        if v.cmd ~= nil then
            M.line_cmds[v.message] = v.cmd
        end
        if v.hl ~= nil then
            vim.api.nvim_buf_add_highlight(M.bufs[bname], -1, v.hl, i-1, 0, -1)
        end
    end
    vim.api.nvim_buf_set_option(M.bufs[bname], 'modifiable', false)
end
...
M.close_win = function(bname)
    vim.api.nvim_win_close(M.wins[bname], true)
end
